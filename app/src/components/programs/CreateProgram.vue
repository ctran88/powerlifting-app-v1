  ...
  export default {
    ...
    methods: {
      ...
      saveProgram: function(status) {
        ...
        // if created is empty, program is new
        if (this.created === '') {
          this.created = new Date().toISOString();
          program.created = this.created;

          // initialize array element for program
          var newProgramRef = this.$firebaseRefs.programs.push();

          // saves program key
          this.id = newProgramRef.toString().match(/-(?:.(?!-))+$/g)[0];
